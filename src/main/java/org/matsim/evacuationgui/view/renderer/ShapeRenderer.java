...
import org.matsim.evacuationgui.model.shape.*;
...
//						this.imageContainer.fillCircle(x-radius/2,y-radius/2,radius,radius);
				...
				{
					Polygon pixelPolygon = ((PolygonShape)shape).getPixelPolygon();
...
//			this.imageContainer.scale(1d/zoomFactor, 1d/zoomFactor);
	...
	public void updatePixelCoordinates(Shape shape)
	{
		...
		else if (shape instanceof PolygonShape)
		{
			...
			org.locationtech.jts.geom.Polygon polygon = ((PolygonShape)shape).getPolygon();
			Polygon pixelPolygon = new Polygon();
