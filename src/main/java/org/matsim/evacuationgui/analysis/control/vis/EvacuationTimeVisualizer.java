	...
	public void processVisualData() {
		...
		for (Cell cell : cells) {
			...
			if (arrivalTime < clusterValues.get(0)) {
				coloration.setAttribute(cell.getId(), org.matsim.evacuationgui.analysis.control.vis.Coloration.getColor(0, colorationMode, cellTransparency));
				continue;
			}
			for (int i = 1; i < k; i++) {
				if ((arrivalTime >= clusterValues.get(i - 1)) && arrivalTime < clusterValues.get(i)) {
					float ik = (float) i / (float) k;
					coloration.setAttribute(cell.getId(), org.matsim.evacuationgui.analysis.control.vis.Coloration.getColor(ik, colorationMode, cellTransparency));
					break;
				}
			}
