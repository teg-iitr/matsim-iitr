...
package org.matsim.evacuationgui.analysis.control.vis;

import org.matsim.api.core.v01.Id;
import org.matsim.api.core.v01.network.Link;
import org.matsim.api.core.v01.population.Person;
import org.matsim.core.utils.collections.Tuple;
import org.matsim.evacuationgui.analysis.control.Clusterizer;
import org.matsim.evacuationgui.analysis.control.vis.Coloration;
		...
		for (Link link : this.links) {
			...
			if ((enterTimes != null) && (enterTimes.size() > 0)) {
				...
				if (enterTime < clusters.get(0).getSecond()) {
					this.coloration.setAttribute(link.getId(), new Tuple<Float, Color>(0f, org.matsim.evacuationgui.analysis.control.vis.Coloration.getColor(0, this.colorationMode, this.cellTransparency)));
					continue;
				}
				for (int i = 1; i < this.k; i++) {
					if ((enterTime >= clusters.get(i - 1).getSecond()) && enterTime < clusters.get(i).getSecond()) {
						float ik = (float) i / (float) this.k;
						this.coloration.setAttribute(link.getId(), new Tuple<Float, Color>(ik, org.matsim.evacuationgui.analysis.control.vis.Coloration.getColor(ik, this.colorationMode, this.cellTransparency)));
						break;
					}
				}
