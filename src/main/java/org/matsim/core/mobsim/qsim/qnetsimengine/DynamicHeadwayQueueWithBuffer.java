...
//		}
	...
	private boolean hasFlowCapacityLeft(VisVehicle veh) {
		...
		return flowcap_accumulate.getValue() > 0.0 || veh.getVehicle().getType().getPcuEquivalents() <= context.qsimConfig.getPcuThresholdForFlowCapacityEasing();
	}
...
//		return false;
		throw new RuntimeException("Part of the code is commented at multiple places, thus, this functionality might not work. Find comments related to attributable vehicles, fix them first.");
	}
