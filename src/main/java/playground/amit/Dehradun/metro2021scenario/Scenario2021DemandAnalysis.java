...
public class Scenario2021DemandAnalysis {
    ...
    private void run(){
        ...
        for(OD od : od_2021_all.values()){
            ...
            DehradunUtils.TravelModesBaseCase2017 mode = DehradunUtils.TravelModesBaseCase2017.valueOf(getMode());
            double tripDist = getTripDistanceInKm(origin, destination, mode);
            od.getAttributes().putAttribute(TRIP_DIST, tripDist);
            double tripTime = getTripTimeInHour(origin, destination, mode);
            od.getAttributes().putAttribute(TRIP_TIME, tripTime);
            ...
            double util_metro_no_asc = UtilityComputation.getUtilMetroWithoutASC(tripDist, tripTime);
        ...
        try(BufferedWriter writer = IOUtils.getBufferedWriter(outFile)){
            writer.write("origin\tdestination\ttotalTrips\tmetroTrips\ttripDistance_km\ttripTime_h\tASC_metro\n");
            for(OD od : od_2021_all.values()){
    ...
    private double getTripDistanceInKm(Coord origin, Coord destination, DehradunUtils.TravelModesBaseCase2017 travelMode){
    ...
    private double getTripTimeInHour(Coord origin, Coord destination, DehradunUtils.TravelModesBaseCase2017 travelMode){
        ...
        double tripTime = getTripDistanceInKm(origin,destination,travelMode);
        switch (travelMode){
            case car:
                tripTime = tripTime/45;
                break;
            case motorbike:
                tripTime = tripTime/70;
                break;
            case bicycle:
                tripTime = tripTime/15;
                break;
            case walk:
                tripTime = tripTime/5;
                break;
