...
public class HaridwarRishikeshScenarioRunner {
    ...
    public void run() {
        ...
        if (runASCCalibration){
            ...
            Metro2021ScenarioASCCalibration metro2021ScenarioASCCalibration = new Metro2021ScenarioASCCalibration();
            metro2021ScenarioASCCalibration.run(outFile_post_ASC_calibration);
        } else {
            // following should not be run with ASC calibration.
            MetroShareEstimator metroShareEstimator= new MetroShareEstimator(hr_scenario);
            metroShareEstimator.run(outFile_post_ASC_calibration, outFile_metro_share);

            MetroTripsComparator metroTripsComparator = new MetroTripsComparator();
