        ...
        try (BufferedReader reader = IOUtils.getBufferedReader(counts_input_data)) {
            ...
            while(line!= null){
                ...
                if (header) {
                    ...
                    int time = Integer.parseInt( parts[labels.indexOf("time")]);
                    double actual_cout = Double.parseDouble( parts[labels.indexOf("count")]);
                    // this is required since two directions traffic is coming on one link; refer to comments in the input file.
                    double additional_count = Double.parseDouble( parts[labels.indexOf("additiona_count")]);
                    count.createVolume(time, actual_cout+additional_count);
                }
